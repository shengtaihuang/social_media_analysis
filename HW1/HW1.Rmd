...
## 三國演義大綱
```{r}
history_tmp=read.csv("三國演義歷史.csv",header=FALSE,stringsAsFactors=FALSE)
...
red_group2 <- red_group %>% gather(word,prop,-chapter) %>% group_by(word) %>% 
  mutate(prop_runmean=runmean(prop,k=3)) %>% left_join(red_chapter,by="chapter") %>% 
...
red_group2 <- red_group %>% gather(word,prop,-chapter) %>% group_by(word) %>% mutate(prop_runmean=runmean(prop,k=3))%>% left_join(red_chapter,by="chapter") %>% 
  mutate(chapter_show=ifelse(prop>=max(prop),chapter_content,""))
