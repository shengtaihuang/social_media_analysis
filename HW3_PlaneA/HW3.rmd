...
## 使用文字分析平台收集03/19-04/04期間，PTT八卦版上討論遊戲「隻狼：暗影雙死」文章
```{r}
#載入原始資料集
...
sekiro_rep <- rep(sample_data$artUrl, sapply(sample_sentences, length))
sekiro %>% head(5)
```
...
```
> 遊戲在設計時考慮到許多現實揮刀打鬥的細節，因此遊戲操作上較為困難，對遊戲公司而言，玩家認為遊戲困難才是具備挑戰性，因此"難"、"困難"、"死"對遊戲公司而言，屬於正向情緒

## 討論遊戲時會出現遊戲類別、背景及角色場景等內容
```{r}
# 隻狼遊戲用詞字典
sekiro_lexicon
```
...
sekiro_words_tf_idf_ns %>% 
  ...
  count(word, sort=TRUE)
```
> 遊戲上市後，有許多遊戲實況主開始玩隻狼，因此出現推薦、討論遊戲實況的文章
